...
impl<'db> ConstraintBound<'db> {
    ...
    fn is_closed(self) -> bool {
        matches!(self, ConstraintBound::Closed(_))
    }

    /// A lower bound is assignable to an upper bound if the underlying types are assignable; and
    /// if either bound is open, if the types are not equivalent.
    fn is_assignable_to(self, db: &'db dyn Db, other: Self) -> bool {
        ...
        self_type.is_assignable_to(db, other_type)
            && ((self.is_closed() && other.is_closed())
                || !self_type.is_equivalent_to(db, other_type))
    }
...
impl<'db> AtomicConstraint<'db> {
    ...
    fn new(
        ...
        if let (ConstraintBound::Closed(lower), ConstraintBound::Closed(upper)) = (upper, lower) {
